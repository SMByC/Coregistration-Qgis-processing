    ...
    try:
        for msg_type, msg_val in load_install_extra_deps():
            ...
            if msg_type == 'log':
                ...
                dialog = WaitDialog(parent, 'Coregistration - installing dependencies')
            elif msg_type == 'install_done':
                dialog.accept()
    except Exception as e:
        if dialog:
            dialog.accept()
        QMessageBox.critical(parent, 'Coregistration - installing dependencies',
                             'An error occurred during the installation of Python packages. ' +
                             'Click on "Stack Trace" in the QGIS message bar for details.')
        raise RuntimeError('\nCoregistration: Error installing Python packages. Read install instruction: '
